from flask import Flask, render_template, jsonify, request, Response, stream_with_context, redirect, url_for, session
...
from src.llm_factory import get_llm_factory, validate_api_key as llm_validate_api_key
from src.database import (
    init_db, User, save_chat_history, get_chat_history, clear_chat_history,
    save_api_key, get_api_key, list_api_keys, delete_api_key,
    set_active_provider, get_active_provider,
    save_document_metadata, get_all_documents, get_document_by_id,
    update_document, delete_document, log_admin_action
)
from src.logging_config import setup_logging, metrics
    ...
    if request.method == "POST":
        ...
        if User.verify_password(username, password):
            ...
            app.logger.warning('Failed login attempt', extra={
                'username': username[:30], 'ip': request.remote_addr
            })
...
EMERGENCY_KEYWORDS = [
    'emergency', '911', 'urgent', 'heart attack', 'stroke', 'chest pain',
    'can\'t breathe', 'cannot breathe', 'difficulty breathing', 'choking',
    'severe bleeding', 'heavy bleeding', 'unconscious', 'passed out',
    'suicide', 'suicidal', 'kill myself', 'overdose', 'seizure',
    'severe pain', 'broken bone', 'head injury', 'allergic reaction'
]
        ...
        def generate():
            try:
                if retriever is None:
                    yield f"data: {json.dumps({'token': 'Knowledge base index not ready. Please ask an admin to run store_index.py.', 'error': True})}\n\n"
                ...
                if confidence < 0.5:
                    yield f"data: {json.dumps({'token': warning_message or 'Insufficient information available.'})}\n\n"
                    yield f"data: {json.dumps({'token': ' Please consult a healthcare professional for accurate information.'})}\n\n"
                    yield f"data: {json.dumps({'done': True, 'confidence': confidence})}\n\n"
                    return
                ...
                # Add disclaimer
                disclaimer = "\n\n⚠️ Medical Disclaimer: This information is for educational purposes only and should not replace professional medical advice, diagnosis, or treatment. Always consult with a qualified healthcare provider for medical decisions."
                ...
                app.logger.info(f"Response generated successfully", extra={
...
def admin_reindex():
    ...
    try:
        ...
        if result.returncode == 0:
            app.logger.info("Index rebuilt successfully", extra={
                'user_id': current_user.id,
...
def dashboard_save_key():
    ...
    if success:
        log_admin_action(current_user.id, 'save_api_key', f'provider={provider}',
                         request.remote_addr)
...
def dashboard_delete_key(provider):
    ...
    if success:
        log_admin_action(current_user.id, 'delete_api_key', f'provider={provider}',
                         request.remote_addr)
...
def dashboard_activate_key(provider):
    ...
    if success:
        log_admin_action(current_user.id, 'activate_provider', f'provider={provider}',
                         request.remote_addr)
        ...
        for fname in os.listdir(data_dir):
            if fname.lower().endswith('.pdf'):
                ...
                if fname in db_docs:
                    ...
                    disk_files.append({
                        'id': None, 'filename': fname, 'original_name': fname,
                        'file_path': fpath, 'file_size': size, 'page_count': 0,
                        'is_active': True, 'status': 'untracked', 'on_disk': True,
                        'created_at': None
                    })
...
def dashboard_delete_doc(doc_id):
    ...
    log_admin_action(current_user.id, 'delete_document',
                     f"filename={doc['filename']}", request.remote_addr)
    return jsonify({'success': True})
