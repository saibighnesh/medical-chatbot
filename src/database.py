...
def get_api_key(provider_name, decrypt=True):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            'SELECT api_key_encrypted FROM api_keys WHERE provider_name = ?',
            (provider_name,)
        )
        result = cursor.fetchone()
...
def set_active_provider(provider_name):
    ...
    try:
        ...
        cursor.execute(
            'UPDATE api_keys SET is_active = 1 WHERE provider_name = ?',
            (provider_name,)
        )
        conn.commit()
        return True
...
def get_all_documents(active_only=False):
    ...
    try:
        ...
        return [{
            ...
            'file_path': row[3], 'file_size': row[4], 'page_count': row[5],
            'display_order': row[6], 'is_active': bool(row[7]),
...
def get_document_by_id(doc_id):
    ...
    try:
        ...
        if result:
            return {
                'id': result[0], 'filename': result[1], 'original_name': result[2],
                'file_path': result[3], 'file_size': result[4], 'page_count': result[5],
                'display_order': result[6], 'is_active': bool(result[7]), 'status': result[8]
            }
        return None
    except Exception as e:
