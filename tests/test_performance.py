...
class TestResponseTime:
    ...
    def test_mock_rag_chain_response_time(self):
        ...
        with patch("app.rag_chain") as mock_chain:
            mock_chain.invoke.return_value = {"answer": "Test response about diabetes."}
...
class TestConcurrentUsers:
    ...
    def test_concurrent_api_requests_mock(self):
        ...
        with patch("app.rag_chain") as mock_chain:
            ...
            with patch("app.download_hugging_face_embeddings"), patch("app.FAISS"), patch("app.ChatGoogleGenerativeAI"):
                from app import app
    ...
    def test_concurrent_retrieval_operations(self):
        ...
        queries = ["diabetes", "symptoms", "treatment", "causes", "prevention"]
        ...
        def search_query(query):
            ...
            docs = retriever.invoke(query)
            end_time = time.time()
            return {"query": query, "time": end_time - start_time, "num_results": len(docs), "success": len(docs) > 0}
    ...
    def test_sustained_load_mock(self):
        ...
        with patch("app.rag_chain") as mock_chain:
            ...
            for i in range(num_requests):
                ...
                results.append({"success": response.status_code == 200, "time": req_end - req_start})
            end_time = time.time()
